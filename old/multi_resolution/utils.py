...
def resize_and_tile_image(image, output_size):
    # Charger l'image
    # image = Image.open(image_path)

    # Obtenir les dimensions initiales de l'image
    ...
    repeat_x = original_size[0] // output_size[0] + 1
    repeat_y = original_size[1] // output_size[1] + 1
    ...
    tiled_image_tensor = tiled_image_tensor[
...
# resize_and_tile_image('grid_0124.jpg', (50,50))
im = Image.open("test.png")
resize_and_tile_image(im, (50, 50))
...
def prep_img_file(image: str, size=None, mean=MEAN):
    ...
    texture = resize_and_tile_image(im, size)
...
def prep_img_tensor(tensor: torch.Tensor, size=None, mean=MEAN):
    ...
    if size is not None:
        tensor = resize(tensor, size, mode="bilinear").unsqueeze(0)
